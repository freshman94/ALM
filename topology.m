...
temp = Sxy;
Sxy2 = Sxy(2,:);    %第二行元素（即节点横坐标）
Sxy2_sort = sort(Sxy2);
for i = 1:NodeAmount
    pos = find(Sxy2==Sxy2_sort(i)); %返回Sxy2中满足条件的元素的位置
    if length(pos)>1
        error('仿真故障，请重试！');
    end
    temp(1,i) = i;
...
%画节点
if PlotIf == 1
    plot(Sxy(2,:),Sxy(3,:),'ko','MarkerEdgeColor','b','MarkerFaceColor','g','MarkerSize',5);    %'ko'：黑色圆圈；'MarkerEdgeColor'：标记的边框颜色；'MarkerFaceColor'：标记的颜色
    %设置图形显示范围
    xlim([0,BorderLength]);
    ylim([0,BorderLength]);
    hold on;    %表示可在原图上修改
